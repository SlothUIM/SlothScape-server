...
//import server.model.entity.player.Right;
import server.net.PacketBuilder;
//import server.punishments.PunishmentType;
//import server.punishments.Punishments;
...
public class RS2LoginProtocol extends FrameDecoder {
	...
	protected Object decode(ChannelHandlerContext ctx, Channel channel, ChannelBuffer buffer) throws Exception {
		...
		case LOGGING_IN:
			...
			if (loginPacketSize <= buffer.capacity()) {
				...
				final String name = Misc.getRS2String(buffer); //Misc.formatPlayerName(Misc.getRS2String(rsaBuffer));
				final String pass = Misc.getRS2String(buffer);
				final String macAddress = Misc.getRS2String(buffer);
				final String identity = Misc.getRS2String(buffer);
				...
				// NEW: Only replace the decoder if the login was actually successful!
				Player p = login(channel, inCipher, outCipher, version, name, pass, macAddress, identity);
				if (p != null) {
	...
	private static Player login(Channel channel, ISAACCipher inCipher, ISAACCipher outCipher, int version, String name, String pass, String macAddress, String identity) {
		...
		if (returnCode == 2) {
			...
			// FIX: Run this for EVERYONE except failed passwords
			if (load != 3) {
		...
		if (returnCode == 2) {
			...
			// --- The 12 "Packet 176" Bytes ---
			bldr.put((byte) player.daysSinceRecovChange); // daysSinceRecovChange (1 Byte)
			bldr.putShort(player.unreadMessages);   // unreadMessages (2 Bytes / Short)
			bldr.put((byte) 1);  // members (1 Byte)
			bldr.putInt(1);      // 'i' / anInt1193 (4 Bytes / Int)
			bldr.putShort(player.daysSinceLastLogin);    // daysSinceLastLogin (2 Bytes / Short)
			bldr.putShort(player.isEmailRegistered ? 1 : 0); // EmailRegistered (2 Bytes / Short)
		...
		return player;
		    ...
		    System.out.println("Exception while creating player: " + e);
		    e.printStackTrace();
		    return null;
		}
	}
