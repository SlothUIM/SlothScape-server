package server.model.multiplayer_session.duel;
...
public class DuelSession extends MultiplayerSession {
	...
	public void accept(Player player, Player recipient, int stageId) {
		...
		case MultiplayerSessionStage.OFFER_ITEMS:
			if (System.currentTimeMillis() - lastRuleModification < 5_000) {
				...
				player.getPA().sendFrame126("A rule was changed in recently, you cannot accept yet.", 6684);
				return;
			}
			if (recipient.getItems().freeSlots() < getItems(player).size()) {
				player.getPA().sendFrame126("You have offered more items than " + recipient.playerName + " has free space.", 6684);
				recipient.getPA().sendFrame126("You do not have enough inventory space to continue.", 6684);
				return;
			}
			if (player.hasNpc || recipient.hasNpc) {
				player.getPA().sendFrame126("You or your opponent has a pet spawned, pick it up.", 6684);
				recipient.getPA().sendFrame126("You or your opponent has a pet spawned, pick it up.", 6684);
				return;
			}
			if (player.getItems().isWearingItem(10501, 3) || recipient.getItems().isWearingItem(10501, 3)) {
				player.getPA().sendFrame126("You or your opponent is wearing a faulty weapon, take it off.", 6684);
				recipient.getPA().sendFrame126("You or your opponent is wearing a faulty weapon, take it off.", 6684);
				return;
			}
			if (recipient.getItems().freeSlots() < getDisabledEquipmentCount(recipient)) {
				player.getPA().sendFrame126("Player doesn't have enough space to unequip the disabled items.", 6684);
				recipient.getPA().sendFrame126("Not enough space to remove the disabled equipped items.", 6684);
				return;
			}
			if (rules.contains(Rule.NO_MELEE) && rules.contains(Rule.NO_MAGE) && rules.contains(Rule.NO_WEAPON)) {
				player.getPA().sendFrame126("You cannot have no melee, no mage and no weapon selected.", 6684);
				recipient.getPA().sendFrame126("You cannot have no melee, no mage and no weapon selected.", 6684);
				return;
			}
			for (Player p : players) {
				...
				if (overlap != null) {
					...
					getOther(p).getPA().sendFrame126("The other player has offered too many of one item, they must remove some.", 6684);
					return;
				}
			}
	...
	public void updateOfferComponents() {
		for (Player player : items.keySet()) {
			...
			player.getPA().sendFrame126("Dueling with: " + recipient.playerName + " (level-" + recipient.combatLevel + ")", 6671);
		}
	}
	...
	public void updateMainComponent() {
		if (stage.getStage() == MultiplayerSessionStage.OFFER_ITEMS) {
			for (Player player : players) {
				...
				player.getPA().sendFrame126("Dueling with: " + recipient.playerName + " (level-" + recipient.combatLevel + ")", 6671);
				player.getPA().sendFrame126("Whip/DDS", 669);
				player.getPA().sendFrame126("", 6684);
			...
			for (Player player : players) {
				...
				player.getPA().sendFrame126("Boosted stats will be reset.", 8238);
				int offset = 0;
				for (Rule rule : rules.rules) {
					if (!rule.getDetails().isEmpty()) {
						player.getPA().sendFrame126(rule.getDetails(), 8242 + offset);
						offset++;
					}
				}
	...
	public void toggleRule(Player player, Rule rule) {
		...
		if (rule.equals(Rule.NO_MELEE) || rule.equals(Rule.NO_RANGE) || rule.equals(Rule.NO_MAGE)) {
			...
			if (count >= 2 && !rules.contains(rule)) {
				player.getPA().sendFrame126("You must fight with at least one combat style.", 6684);
				return;
			}
		}
