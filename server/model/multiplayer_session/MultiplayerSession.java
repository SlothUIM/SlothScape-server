		...
		if (item.isStackable()) {
			for (int i = 0; i < items.size(); i++) {
				if (items.get(i).getId() == id) {
					long total = ((long) amount + (long) player.getItems().getItemCount(id, false));
					if (total > Integer.MAX_VALUE) {
						amount = Integer.MAX_VALUE - player.getItems().getItemCount(id, false);
					}
	...
	public void refreshItemContainer(Player player, Player other, int containerId) {
		...
		if (len > 0) {
			for (Item item : items.get(other)) {
				...
				if(item.getId() > 0) {
					player.outStream.writeWordBigEndianA(item.getId() + 1);
				} else {
