...
//import server.model.entity.npc.animations.BlockAnimation;
import server.model.players.Player;
//import server.model.entity.player.combat.magic.MagicData;
...
public class MeleeData {
	...
	public static int getAttackSound(Player c, int weaponID) {
	    String itemName = c.getItems().getItemName(weaponID);
	    if (itemName.contains("dragon dagger")) {
	        return Sound.SOUND_LIST.ATTACK_DDS.getSound();
	    } else if (itemName.contains("sword") || itemName.contains("dagger") && (c.fightMode == 0 || c.fightMode == 2) && !itemName.contains("dragon dagger")) {
	        return Sound.SOUND_LIST.STABSWORD_STAB.getSound();
	    } else if (itemName.contains("sword") && c.fightMode == 1) {
	        return Sound.SOUND_LIST.STABSWORD_SLASH.getSound();
	    }  else if (itemName.contains("Abyssal_whip")) {
	        return Sound.SOUND_LIST.WHIP.getSound();
	    } else if (itemName.contains("rapier")) {
	        return Sound.SOUND_LIST.ATTACK_DAGGER.getSound();
	    } else if (itemName.contains("scimitar")) {
	        return Sound.SOUND_LIST.ATTACK_SCIMITAR.getSound();
	    } else if (itemName.contains("longsword") && (c.fightMode == 0 || c.fightMode == 2)) {
	        return Sound.SOUND_LIST.STABSWORD_SLASH.getSound();
	    } else if (itemName.contains("battleaxe")) {
	        return Sound.SOUND_LIST.BAXE_CRUSH.getSound();
	    } else if (itemName.contains("warhammer")) {
	        return Sound.SOUND_LIST.WARHAMMER_CRUSH.getSound();
	    } else if (itemName.contains("halberd")) {
	        return Sound.SOUND_LIST.ATTACK_HALBERD.getSound();
	    } else if ((itemName.contains("shortbow") || itemName.contains("bow") || itemName.contains("Shortbow")) && !itemName.contains("longbow")) {
	        return Sound.SOUND_LIST.SHORTBOW.getSound();
	    } else if (itemName.contains("longbow")) {
	        return Sound.SOUND_LIST.LONGBOW.getSound();
	    } else if (itemName.contains("spear")) {
	        return Sound.SOUND_LIST.SPEAR_ATTACK.getSound();
	    } else if (itemName.contains("staff")) {
	        return Sound.SOUND_LIST.STAFF_CRUSH.getSound();
	    } else if (itemName.contains("scythe of vitur")) {
	        return Sound.SOUND_LIST.SCYTHE_SLASH.getSound();
	    } else {
	        for (Sound.SOUND_LIST sound : Sound.SOUND_LIST.values()) {
	            if (sound.weapon == weaponID) {
	                return sound.weaponSound;
	            }
	        }
	        return 2564+c.fightMode;
	    }
	}
	public static int getDefSound(Player c, int weaponID) {
	    String shieldName = c.getItems().getItemName(weaponID).toLowerCase();

	    if (shieldName.contains("kiteshield")) {
	        return 12;
	    } else if (shieldName.contains("sq shield")) {
	        return 13;
	    } else if (shieldName.contains("defender")) {
	        return 28;
	    }
...
//	}
	...
	if (c.playerEquipment[c.playerRing] == 33743) {
		...
		case 7158:
			...
			case 10024:
				c.playerStandIndex = 5160;
				c.playerRunIndex = 5168;
				c.playerWalkIndex = 5164;
				c.playerTurnIndex = 5165;
				c.playerTurn180Index = 5165;
				c.playerTurn90CWIndex = 5166;
				c.playerTurn90CCWIndex = 5167;
	...
	public static int getWepAnim(Player c, String weaponName) {
		...
		if (weaponName.contains("sword") && !weaponName.contains("training") || weaponName.contains("scimitar")) {
			if(c.fightMode == 0)
				return 390;
			if(c.fightMode == 1)
				return 386;
	...
	public static int npcDefenceSound(int i) {
		return Sound.handleBlockSound(i);
