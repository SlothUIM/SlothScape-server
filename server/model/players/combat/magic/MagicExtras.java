		...
		if (NPCHandler.npcs[npcId] != null) {
			...
			if (checkMultiBarrageReqsNPC(npcId)) {
				...
				if (Misc.random(c.getCombat().mageAtk()) > Misc.random(NPCHandler.npcs[npcId].defence) && !c.magicFailed) {
					...
					if (n.npcType == 7413) {
						n.appendDamage(c.getCombat().magicMaxHit(), Hitmark.HIT);
					} else {
	...
	public static void multiSpellEffect(Player c, int playerId, int damage) {
		...
		case 13023:
			if (System.currentTimeMillis() - PlayerHandler.players[playerId].reduceStat > 35000) {
				...
				//plr.getPA().decreaseLevelOrMin((plr.getPA().getLevel(Skill.ATTACK) * 10) / 100, Skill.ATTACK);
			}
			break;
		case 12919: // blood spells
		case 12929:
		...
		if (PlayerHandler.players[playerId] != null) {
			...
			if (c.getCombat().checkMultiBarrageReqs(playerId)) {
				...
				if (Misc.random(c.getCombat().mageAtk()) > Misc.random(c2.getCombat().mageDef()) && !c.magicFailed) {
					...
					c.getPA().addSkillXP((MagicData.MAGIC_SPELLS[c.oldSpellId][7] + damage * 1/*(c.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.MAGIC_EXP_RATE)*/), 6);
					c.getPA().addSkillXP((MagicData.MAGIC_SPELLS[c.oldSpellId][7] + damage * 1/*(c.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.MAGIC_EXP_RATE)*/ / 3), 3);
