...
//import server.content.events.WildernessEscape;
import server.event.CycleEvent;
import server.event.CycleEventContainer;
import server.event.CycleEventHandler;
import server.model.npcs.NPCHandler;
//import server.model.npcs.pets.PetHandler.Pets;
...
import server.model.players.combat.effects.AmuletOfTheDamnedDharokEffect;
import server.model.players.combat.effects.AmuletOfTheDamnedKarilEffect;
import server.model.players.combat.effects.SerpentineHelmEffect;
...
public class AttackPlayer {
	...
	public static void addCombatXP(Player c, CombatType type, int damage) {
		...
		// OSRS Base XP values per 1 damage point
		double hpXp = damage * 1.33;
		double baseCombatXp = damage * 4.0;
		double magicCombatXp = damage * 2.0;

		switch (type) {
			case MELEE:
				int meleeHp = (int) (hpXp * Config.MELEE_EXP_RATE);
				c.getPA().addSkillXP(meleeHp, Skill.HITPOINTS.getId());
				skills.add(Skill.HITPOINTS.getId());
				experience += meleeHp;

				switch (c.fightMode) {
					case 0: // Accurate
						int atkXp = (int) (baseCombatXp * Config.MELEE_EXP_RATE);
						c.getPA().addSkillXP(atkXp, Skill.ATTACK.getId());
						skills.add(Skill.ATTACK.getId());
						experience += atkXp;
						break;
					case 1: // Block / Defensive
						int defXp = (int) (baseCombatXp * Config.MELEE_EXP_RATE);
						c.getPA().addSkillXP(defXp, Skill.DEFENCE.getId());
						skills.add(Skill.DEFENCE.getId());
						experience += defXp;
						break;
					case 2: // Aggressive
						int strXp = (int) (baseCombatXp * Config.MELEE_EXP_RATE);
						c.getPA().addSkillXP(strXp, Skill.STRENGTH.getId());
						skills.add(Skill.STRENGTH.getId());
						experience += strXp;
						break;
					case 3: // Controlled
						int sharedXp = (int) ((damage * 1.33) * Config.MELEE_EXP_RATE);
						c.getPA().addSkillXP(sharedXp, Skill.ATTACK.getId());
						c.getPA().addSkillXP(sharedXp, Skill.STRENGTH.getId());
						c.getPA().addSkillXP(sharedXp, Skill.DEFENCE.getId());

						skills.add(Skill.ATTACK.getId());
						skills.add(Skill.STRENGTH.getId());
						skills.add(Skill.DEFENCE.getId());
						experience += (sharedXp * 3);
						break;
				}
				break;

			case RANGE:
				// Assuming you have a Config.RANGE_EXP_RATE (If not, just use MELEE_EXP_RATE)
				int rangeHp = (int) (hpXp * Config.RANGE_EXP_RATE);
				c.getPA().addSkillXP(rangeHp, Skill.HITPOINTS.getId());
				skills.add(Skill.HITPOINTS.getId());
				experience += rangeHp;
				...
				switch (c.fightMode) {
					case 0: // Accurate
					case 2: // Rapid
						int rangeXp = (int) (baseCombatXp * Config.RANGE_EXP_RATE);
						c.getPA().addSkillXP(rangeXp, Skill.RANGED.getId());
						skills.add(Skill.RANGED.getId());
						experience += rangeXp;
						break;
					case 1: // Longrange
					case 3:
						// OSRS Longrange splits exactly 2 XP to Range and 2 XP to Defence
						int splitRange = (int) ((damage * 2.0) * Config.RANGE_EXP_RATE);
						int splitDef = (int) ((damage * 2.0) * Config.RANGE_EXP_RATE);

						c.getPA().addSkillXP(splitRange, Skill.RANGED.getId());
						c.getPA().addSkillXP(splitDef, Skill.DEFENCE.getId());

						skills.add(Skill.RANGED.getId());
						skills.add(Skill.DEFENCE.getId());
						experience += (splitRange + splitDef);
						break;
				}
				break;

			case MAGE:
				// Assuming you have a Config.MAGIC_EXP_RATE
				int mageHp = (int) (hpXp * Config.MAGIC_EXP_RATE);
				c.getPA().addSkillXP(mageHp, Skill.HITPOINTS.getId());
				skills.add(Skill.HITPOINTS.getId());
				experience += mageHp;

				int magicXp = (int) (magicCombatXp * Config.MAGIC_EXP_RATE);
				c.getPA().addSkillXP(magicXp, Skill.MAGIC.getId());
		...
		if (experience > 0) {
			// Uncomment this once your XP drops are ready!
			// c.getPA().sendExperienceDrop(true, experience, ArrayUtils.toPrimitive(skills.toArray(new Integer[0])));
		}
	}
	...
	public static void playerDelayedHit(final Player c, final int targetId, final Damage d) {
		if (PlayerHandler.players[targetId] != null) {
			...
			//}
			if (target.attackTimer <= 4 || target.attackTimer == 0 && target.playerIndex == 0 && !c.usingMagic) { // block
																									// animation
		...
		if (Objects.nonNull(attacker) && Objects.nonNull(defender)) {
			if (combatType.equals(CombatType.MELEE)) {
				...
				if (attacker.magicDef) {
					...
					attacker.getPA().addSkillXP((damage * (Config.MELEE_EXP_RATE) / 3), 1);
