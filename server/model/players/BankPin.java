...
public class BankPin {
	...
	public void pinSettingFrames() {
		...
		client.getPA().sendFrame126("Customers are reminded", 15040);
		client.getPA().sendFrame126("that they should NEVER", 15041);
		client.getPA().sendFrame126("tell anyone their Bank", 15042);
		client.getPA().sendFrame126("PINs or Passwords, nor", 15043);
		client.getPA().sendFrame126("should they ever enter ", 15044);
		client.getPA().sendFrame126("their PINs on any", 15045);
		client.getPA().sendFrame126("website form", 15046);
		if (!client.hasBankpin) {
			client.getPA().sendFrame126("Change your PIN", 15078);
			client.getPA().sendFrame126("Delete your PIN", 15082);
			client.getPA().sendFrame126("Change your recovery delay", 15080);
	...
	public void openPin() {
		...
		randomizeNumbers();
		client.getPA().sendFrame126("First click the FIRST digit",
				15313);
	...
	private void sendPins() {
		...
		for (int i = 0; i < getBankPins().length; i++) {
			client.getPA().sendFrame126("" + getBankPins()[i],
					stringIds[i]);
		}
	}

	private void handleButtonOne(int button) {
		client.getPA().sendFrame126("Now click the SECOND digit",
				15313);
	...
	private void handleButtonTwo(int button) {
		client.getPA().sendFrame126("Now click the THIRD digit",
				15313);
	...
	private void handleButtonThree(int button) {
		client.getPA().sendFrame126("Now click the LAST digit",
				15313);
