			...
			if (player.tutorialProgress == 0) { // wrench
				...
				player.getDH()
						.chatboxText(
								...
								"Cut down a tree");
				player.getPA().createObjectHints(3099, 3095, player.getHeight(), 2);
				player.getPA().chatbox(6179);
				...
				player.getPA().chatbox(6179);
				player.getPA().createPlayerHints(1, 2);
			} else if (player.tutorialProgress == 8) {

				player.getPA().chatbox(6180);
				player.getDH().chatboxText("This is the base for many of the meals. To make dough we must", "mix flour and water. First right click the bucket of water and", "select use, then left click on the pot of flour.", "", "Making dough");

				player.getPA().chatbox(6179);player.getPA().removeHintIcon(player);
				...
				player.getDH()
						.chatboxText(
								...
								"The music player");
				player.getPA().createObjectHints(3073, 3090, player.getHeight(), 2);
				...
				player.getPA().createObjectHints(3111, 9518, player.getHeight(), 2);
			} else if (player.tutorialProgress == 29) { // Prayer
				player.getDH().sendDialogues(3092, 222);
				...
				player.setSidebarInterface(9, 5715);
				player.getPA().flashSideBarIcon(-9);
				...
				player.getDH().sendDialogues(3108, 946);
			}
			break;
