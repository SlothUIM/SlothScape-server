...
import server.model.path.PathGenerator;
...
public class EntityFollowEvent implements CycleEvent {
	...
	public void execute(CycleEventContainer container) {
		...
		if (!this.entity.getLocation().withinDistance(this.following.getLocation(), 31)) {
			...
			if (this.entity.isNPC()) {
				...
				//Handle pet walking
				if (Pets.isPet(npc.getDefinition().getNpcId())) {
					//Move pet to owner
		...
		if (destination != null) {
		    RS317PathFinder.findPath(this.entity, destination.getX(), destination.getY(), false, 31, 31);
		}
