...
public class SmithingInterface {
	...
	public void showSmithInterface(int itemId) {
		...
		{
			c.sendMessage("You need an hammer to do that.");
			return;
		}
        SmithBar pref = SmithBar.byItemId(itemId);
        if(pref != null)
		if(pref.level <= c.getSkills().getLevel(Skill.SMITHING)) {
			if (itemId == 2349 && c.tutorialProgress == 20) {
				c.getPA().chatbox(6180);
				c.getDH().chatboxText("Now you have the Smithing menu open, you will see a list of all", "the things you can make. Only the dagger can be made at your", "skill level, this is shown by the white text under it. You'll need", "to select the dagger to continue.", "Smithing a dagger");
			...
			c.sendMessage("You need a smithing level of "+pref.level+" to make "+c.getItems().getItemName(itemId).trim().replace("bar", "").toLowerCase()+"items.");
	...
	private void makeIronInterface(Player c) {
		...
		c.getPA().sendFrame34(3096, 3, 1123, 1);
		c.getPA().sendFrame126("", 1135);
		c.getPA().sendFrame126("", 1134);
		c.getPA().sendFrame126("", 1132);
		c.getPA().sendFrame126("", 1096);
	...
	private void makeBronzeInterface(Player c) {
		...
		c.getPA().sendFrame34(3095, 3, 1123, 1);
		c.getPA().sendFrame126("", 1135);
		c.getPA().sendFrame126("", 1134);
		c.getPA().sendFrame126("", 11461);
		c.getPA().sendFrame126("", 11459);
	...
	private String GetForBarsAndLevel(int i, int j, Player c, int level) {
		if (c.getItems().playerHasItem(i, j) && c.getSkills().getLevel(Skill.SMITHING) >= level)
			return "@gre@";
