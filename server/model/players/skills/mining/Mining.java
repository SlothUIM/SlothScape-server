...
public class Mining {
	...
	public void obtainGem(Player player) {
		...
		if (giveGem(player)) {
			...
			player.sendMessage("You found an " + ItemAssistant.getItemName(reward) + ".");
		}
	}
	...
	public static enum rockData {
		...
		CLAY(new int[] { 2108, 2109, 11189, 11190, 11191, 14905, 14904 }, 1, 5, 1, 2, new int[] { 434 }),
		COPPER(new int[] { 3042, 2091, 2090, 11960, 14906, 14907, 10079 }, 1, 18, 1, 4, new int[] { 436 }),
		TIN(new int[] { 2094, 2095, 3043, 11958, 11957, 11959, 11933, 11934, 11935, 14903, 14902, 10080 }, 1, 18, 1, 4, new int[] { 438 }),
		BLURITE(new int[] { 10574, 10583, 10584, 2110 }, 10, 20, 1, 42, new int[] { 668 }),
		IRON(new int[] { 450, 2093, 2092, 11962, 11956, 11954, 14856, 14857, 14858, 14914, 14913 }, 15, 35, 2, 9, new int[] { 440 }),
		SILVER(new int[] { 2101, 11186, 11187, 11188, 2100 }, 20, 40, 3, 100, new int[] { 442 }),
		COAL(new int[] { 2096, 11963, 11964, 14850, 14851, 14852, 11930, 11931, 11932 }, 30, 50, 4, 50, new int[] { 453 }),
		GOLD(new int[] { 2099, 2098, 11183, 11184, 11185 }, 40,	65, 6, 100, new int[] { 444 }),
	...
	public void startMining(final Player player, final int objectID, final int objectX, final int objectY, final int type) {
		...
		if (rock.getRequiredLevel() > miningLevel) {
			player.sendMessage("You need a Mining level of " + rock.getRequiredLevel() + " to mine this rock.");
			return;
		}
		// check id the player has a pickaxe they can use on them
		for (int i = 0; i < Pick_Settings.length; i++) {
			if (player.getItems().playerHasItem(Pick_Settings[i][0]) || player.playerEquipment[player.playerWeapon] == Pick_Settings[i][0]) {
		...
		if (player.getItems().freeSlots() < 1) {
			player.sendMessage("You do not have enough inventory slots to do that.");
			return;
		}

		player.startAnimation(Pick_Settings[player.miningAxe][3]);
		player.getPA().sendSound(432, 100, 0);
		...
		if (player.tutorialProgress == 17 || player.tutorialProgress == 18) {
			...
			player.getDH().chatboxText("", "Your character is now attempting to mine the rock.", "This should only take a few seconds.", "", "Please wait");
		...
		CycleEventHandler.getSingleton().addEvent("miningEvent".hashCode(), player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				if (player.isMining) {
					if (!giveGem(player)) {
						player.getItems().addItem(oreID, 1);
						player.getPA().addSkillXP(rock.getXp(), player.playerMining);
						player.sendMessage("You manage to mine some " + ItemAssistant.getItemName(oreID).toLowerCase() + ".");
					} else {
						obtainGem(player);
					}
				}
				if (player.tutorialProgress == 17) {
					if (rock != rockData.TIN) {
						player.getDH().sendStatement("You should mine tin first.");
						resetMining(player);
						return;
					}
					if (player.getItems().playerHasItem(438)) {
						player.getPA().createObjectHints(3086, 9501, player.getHeight(), 2);
						player.getDH().chatboxText("Now you have some tin ore you must need some copper ore, then", "you'll have all you need to create a bronze bar. As you did before", "riger click on the copper rock and select 'mine'.", "", "Mining");
						player.tutorialProgress = 18;
					}
				} else if (player.tutorialProgress == 18) {
					if (rock != rockData.COPPER) {
						player.getDH().sendStatement("You have already mined this type of ore, now try the other.");
						resetMining(player);
						return;
					}
					if (player.getItems().playerHasItem(436)) {
						player.getPA().createObjectHints(3078, 9495, 0, 2);
						player.getDH().sendDialogues(3061, -1);
					}
				}
				if (player.getItems().freeSlots() < 1) {
					player.sendMessage("You have ran out of inventory slots.");
					container.stop();
				}
	...
	public void mineRock(int respawnTime, int x, int y, int type, int i) {
		if (i != 2491) {
			new Object(10081, x, y, 0, type, 10, i, respawnTime);
			   ...
			   CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
					...
					if (c.tutorialProgress == 15) {
						...
						.chatboxText(
								...
								"the brown ones next.", "",
								"It's tin");
						c.getPA().createObjectHints(3086, 9501,
								c.getHeight(), 2);
						c.getPA().chatbox(6179);
