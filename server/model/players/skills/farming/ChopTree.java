...
public class ChopTree {
	...
	static {
		...
		trees[2] = new Tree(new int[] { 1281, 2037 }, 15, 37.5, 1521, 1356, 14,
				25);
		trees[3] = new Tree(new int[] { 1308, 5551, 5552}, 30, 67.5,
				1519, 7399, 14, 5);
	...
	public static int getAxeIndex(Player player) {
		for (int i = 0; i < axes.length; i++) {
			if (player.playerEquipment[player.playerWeapon] == (axes[i].getId())) {
				return i;
			}
		}
		for (int i = axes.length - 1; i >= 0; i--) {
			if (player.getItems().playerHasItem(axes[i].getId())) {
				if (player.playerLevel[player.playerWoodcutting] >= axes[i].getLevel()) {
					return i;
				}
			}
		}
