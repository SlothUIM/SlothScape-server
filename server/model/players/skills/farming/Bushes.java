...
public class Bushes {
	...
	public Bushes(Player player2) {
		this.player = player2;
	}

	// Farming data
	public int[] bushesStages = new int[5];
	public int[] bushesSeeds = new int[5];
	public int[] bushesState = new int[5];
	public long[] bushesTimer = new long[5];
	public double[] diseaseChance = { 1, 1, 1, 1, 1 };
	public boolean[] hasFullyGrown = { false, false, false, false, false };
	public boolean[] bushesWatched = { false, false, false, false, false };
	...
	public static final int MAIN_BUSHES_CONFIG = 529;
	...
	public enum BushesFieldsData {
		...
		FARMINGGUILD(4, new Point[] { new Point(1260, 3733), new Point(1261, 3734) }, 2336);
	...
	public void updateBushesStates() {
		...
		configValue = (configValues[configValue2] << 7);
	...
	public void doCalculations() {
		for (int i = 0; i < bushesSeeds.length; i++) {
			if (bushesStages[i] > 0 && bushesStages[i] <= 3
					&& Server.getMinutesCounter() - bushesTimer[i] >= 5) {
				bushesStages[i]--;
	...
	public boolean plantSeed(int objectX, int objectY, final int seedId) {
		...
		if (bushesData.getLevelRequired() > player.playerLevel[player.playerFarming]) {
		...
		if (player.getItems().getItemAmount(bushesData.getSeedId()) < bushesData
				.getSeedAmount()) {
		...
		player.getItems().deleteItem(seedId, bushesData.getSeedAmount());
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
            ...
            public void execute(CycleEventContainer container) {
				...
				player.getPA().addSkillXP((int)bushesData.getPlantingXp(), player.playerFarming);
				container.stop();
			}
		...
		player.startAnimation(832);
		 CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
				...
				if (bushesState[bushesFieldsData.getBushesIndex()] == 3) {
					...
					player.getPA().addSkillXP((int)bushesData.getCheckHealthXp(), player.playerFarming);
					bushesState[bushesFieldsData.getBushesIndex()] = 0;
					hasFullyGrown[bushesFieldsData.getBushesIndex()] = false;
					bushesTimer[bushesFieldsData.getBushesIndex()] = Server.getMinutesCounter() - bushesData.getGrowthTime();
				...
				player.getItems().addItem(bushesData.getHarvestId(), 1);
				player.getPA().addSkillXP((int)bushesData.getHarvestXp(), player.playerFarming);
				bushesTimer[bushesFieldsData.getBushesIndex()] = Server.getMinutesCounter();
				int difference = bushesData.getEndingState()
						- bushesData.getStartingState();
				int growth = bushesData.getGrowthTime();
			...
			CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
	            ...
	            public void execute(CycleEventContainer container) {
					if (bushesStages[bushesFieldsData.getBushesIndex()] - 4 < inspectData
						...
						//player.getDH().sendStatement( inspectData
