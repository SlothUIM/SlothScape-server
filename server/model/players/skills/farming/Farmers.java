...
public class Farmers {
	...
	public static void protectPlant(Client player, int indexArray,
		...
		/*if (stage == 1) {
			if (fieldType == "allotment") {
				...
				if (player.getAllotment().allotmentStages[index] <= 3) {
					//player.getDH().sendNpcChat(player.npcType, ChatEmotes.ANNOYED,  "I am sorry but you have no crops growing in this patch.");
					//player.getDH().endDialogue();
				} else {
					//player.getDH().sendNpcChat(player.npcType, ChatEmotes.NOT_INTERESTED, "If you like, but I want " + payment[1] + " " + getFinalPaymentString(ItemAssistant.getItemName(payment[0]).toLowerCase()) + (getFinalPaymentString(ItemAssistant.getItemName(payment[0]).toLowerCase()).endsWith("s") ? "" : "s") + " for that.");
			...
			if (player.getItems().getItemAmount(payment[0]) < payment[1]) {
				player.getDH().sendNpcChat(player.npcType, ChatEmotes.DEFAULT, "Sorry, but you do not have the required items",
				...
				//player.getDH().sendNpcChat(player.npcType, ChatEmotes.DEFAULT, "Here you go, I will be taking care of this patch",
	...
	public static void chopDownTree(Client player, int npcId) {
		...
		if (player.getItems().getItemAmount(995) < 200) {
			//player.getDH().sendNpcChat(player.npcType, ChatEmotes.DEFAULT, "I am sorry, but you do not have enough money", "to pay me to chop down this tree.");
		} else {
			player.getItems().deleteItem(995, 200);
			//player.getDH().sendNpcChat(player.npcType, ChatEmotes.DEFAULT, "There you go, I have chopped down your tree but I am ", "keeping the logs and roots as compensation.");
	...
	public static void sendFarmingAdvice(Client player) {
		//player.getDH().sendNpcChat(player.npcType, ChatEmotes.HAPPY_JOYFUL, farmingAdvices[Misc.random(farmingAdvices.length - 1)]);
