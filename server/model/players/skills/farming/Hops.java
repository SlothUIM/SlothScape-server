...
public class Hops {
	...
	public static final int MAIN_HOPS_CONFIG = 529;
	...
	public enum HopsFieldsData {
		...
		LUMBRIDGE(0,new Point[] { new Point(3227, 3313), new Point(3231, 3317) },
	...
	public void updateHopsStates() {
		...
		configValue = (configValues[0]) + (configValues[1] << 7);
	...
	public void doCalculations() {
		for (int i = 0; i < hopsSeeds.length; i++) {
			if (hopsStages[i] > 0 && hopsStages[i] <= 3
					&& Server.getMinutesCounter() - hopsTimer[i] >= 5) {
				hopsStages[i]--;
	...
	public boolean plantSeed(int objectX, int objectY, final int seedId) {
		...
		if (hopsData.getLevelRequired() > player.getSkills().getLevel(Skill.FARMING)) {
			//player.getDH().sendStatement("You need a farming level of "
				//			+ hopsData.getLevelRequired()
				//			+ " to plant this seed.");
			player.sendMessage("You need a higher farming level.");
			return true;
		}
		...
		if (player.getItems().getItemAmount(hopsData.getSeedId()) < hopsData
				.getSeedAmount()) {
		...
		player.getItems().deleteItem(seedId, hopsData.getSeedAmount());
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
            ...
            public void execute(CycleEventContainer container) {
				...
				hopsTimer[hopsFieldsData.getHopsIndex()] = Server.getMinutesCounter();
				player.getPA().addSkillXP((int)hopsData.getPlantingXp(), player.playerFarming);
				container.stop();
			}
	...
	public boolean harvest(int objectX, int objectY) {
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
            ...
            public void execute(CycleEventContainer container) {
				...
				player.getPA().addSkillXP((int)hopsData.getHarvestXp(), player.playerFarming);
			}
