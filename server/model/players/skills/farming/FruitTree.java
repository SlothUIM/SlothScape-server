...
public class FruitTree {
	...
	public FruitTree(Player player2) {
		this.player = player2;
	}
	...
	public static final int MAIN_FRUIT_TREE_CONFIG = 529;
	...
	public void updateFruitTreeStates() {
	    ...
	    int configValue = 0;
	    if (configValues.length > 0) configValue |= (configValues[0] & 0xFF);          // patch 1, bits 0-7
	    if (configValues.length > 1) configValue |= (configValues[1] & 0xFF) << 8;    // patch 2, bits 8-15
	    if (configValues.length > 2) configValue |= (configValues[2] & 0xFF) << 16;   // patch 3, bits 16-23
	    if (configValues.length > 3) configValue |= (configValues[3] & 0xFF) << 24;   // patch 4, bits 24-31
	...
	public int getConfigValue(int stage, int saplingId, int state, int index) {
	    FruitTreeData data = FruitTreeData.forId(saplingId);

	    if (stage <= 3) return stage; // early stages: weeds etc.

	    if (data == null) return -1;

	    if (stage >= data.getEndingState()) {
	        hasFullyGrown[index] = true;
	    }

	    if (state == 6) return data.getStumpState(); // dead
	    if (state == 3) return data.getCheckHealthState(); // ready to check health

	    return data.getStartingState() + stage - 4; // generic growth stage
	}
	...
	public boolean plantSapling(int objectX, int objectY, final int saplingId) {
		...
		if (fruitTreeData.getLevelRequired() > player.getSkills().getLevel(Skill.FARMING)) {
			player.getDH().sendStatement(
		...
		player.getItems().removeItem(saplingId, 1);
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
            ...
            public void execute(CycleEventContainer container) {
				...
				fruitTreeTimer[fruitTreeFieldsData.getFruitTreeIndex()] = Server
						.getMinutesCounter();
				player.getPA().addSkillXP((int)fruitTreeData.getPlantingXp(), player.playerFarming);
				container.stop();
			}
		...
		player.startAnimation(832);
		 CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
				...
				if (fruitTreeState[fruitTreeFieldsData.getFruitTreeIndex()] == 3) {
					...
					player.getPA().addSkillXP((int)fruitTreeData.getCheckHealthXp(), player.playerFarming);
					fruitTreeState[fruitTreeFieldsData.getFruitTreeIndex()] = 0;
				...
				player.getItems().addItem(fruitTreeData.getHarvestId(), 1);
				player.getPA().addSkillXP((int)fruitTreeData.getHarvestXp(), player.playerFarming);
				fruitTreeTimer[fruitTreeFieldsData.getFruitTreeIndex()] = Server.getMinutesCounter();
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (fruitTreeStages[fruitTreeFieldsData.getFruitTreeIndex()] == 0) {
			player.getDH().sendStatement("This is a fruit tree patch. The soil has not been treated.",
