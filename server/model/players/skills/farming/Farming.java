...
public class Farming {
	...
	public static boolean prepareCrop(Player player, int item, int id, int x,
		...
		if (item >= 3422 && item <= 3428 && id == 4090) {
			...
			player.getItems().addItem(item + 8, 1);
	...
	public static boolean inspectObject(Player c, int x, int y) {
		// allotments
		if (c.getAllotment().inspect(x, y)) {
			return true;
		} // flowers
		if (c.getFlowers().inspect(x, y)) {
			return true;
		}
		// herbs
		if (c.getHerbs().inspect(x, y)) {
			return true;
		}
		// hops
		if (c.getHops().inspect(x, y)) {
			return true;
		}
		// bushes
		if (c.getBushes().inspect(x, y)) {
			return true;
		}
		// trees
		if (c.getTrees().inspect(x, y)) {
			return true;
		}
		// fruit trees
		if (c.getFruitTrees().inspect(x, y)) {
			return true;
		}
		// special plant one
		if (c.getSpecialPlantOne().inspect(x, y)) {
			return true;
		}
		// special plant two
		if (c.getSpecialPlantTwo().inspect(x, y)) {
			return true;
		}
		return false;
	}
