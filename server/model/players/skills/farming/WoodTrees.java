...
import server.model.players.skills.farming.ChopTree.Tree;
...
public class WoodTrees {
	...
	public WoodTrees(Player player2) {
		this.player = player2;
	}
	...
	public static final int MAIN_TREE_CONFIG = 529;
	...
	public void updateTreeStates() {
	    ...
	    for (int i = 0; i < treeStages.length; i++) {
	       // if (!isTreePatchNearby(i)) //continue;

	        int value = getConfigValue(treeStages[i], treeSaplings[i], treeState[i], i);
	        configValue |= value << (i * 8);
	    }
	...
	public void doCalculations() {
		for (int i = 0; i < treeSaplings.length; i++) {
			if (treeStages[i] > 0 && treeStages[i] <= 3
					&& Server.getMinutesCounter() - treeTimer[i] >= 5) {
				treeStages[i]--;
	...
	public boolean plantSapling(int objectX, int objectY, final int saplingId) {
		...
		if (treeData.getLevelRequired() > player.playerLevel[player.playerFarming]) {
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
            ...
            public void execute(CycleEventContainer container) {
				...
				treeTimer[treeFieldsData.getTreeIndex()] = Server
						.getMinutesCounter();
				player.getPA().addSkillXP((int)treeData.getPlantingXp(), player.playerFarming);
				container.stop();
			}
	...
	public boolean checkHealth(int objectX, int objectY) {
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
            ...
            public void execute(CycleEventContainer container) {
				...
				player.getPA().addSkillXP((int)treeData.getCheckHealthXp(), player.playerFarming);
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (treeState[treeFieldsData.getTreeIndex()] == 1) {
			...
			player.getDH().sendStatement2("This tree is dead. You did not cure it while it was diseased.",
		   ...
		   CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			   final Tree tree = ChopTree.getTree(object);
				...
				/*if (SkillHandler.skillCheck(
						player.playerLevel[player.playerWoodcutting],
						treeLevel, axeChance * 20)) {
					player.getItems().addItem(ChopTree.getTree(object).getLog(), 1);
					player.sendMessage(
							"You get some "
									+ ItemAssistant
											.getItemName(ChopTree.getTree(object)
													.getLog())
											.toLowerCase() + ".");
					player.getPA().addSkillXP((int)ChopTree.getTree(object).getXP(), player.playerWoodcutting);
