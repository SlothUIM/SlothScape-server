...
//import server.model.players.RightGroup;
...
public class Slayer {
	...
	public void createNewTask(int masterId) {
		SlayerMaster.get(masterId).ifPresent(m -> {
			if (player.calculateCombatLevel() < m.getLevel()) {
				...
				player.getDH().npcChat(master, "Doric", Anim.CALM_2.getAnimationId(), 
						"You need a combat level of " + m.getLevel() + " to receive tasks from me.", "Please come back when you have this combat level.");
			...
			if(task.isPresent()) {
				...
				player.getDH().npcChat(m.getId(), "Kuradel", Anim.CALM_2.getAnimationId(), 
						"You have been assigned " + taskAmount + " " + task.get().getPrimaryName() + ".",
				...
				master.ifPresent(m -> {
					...
					case 6797:
						if (Misc.random(10) == 1 && (player.getItems().isWearingItem(21183)))  {
							...
							player.getPA().addSkillXP(task.getExperience() * 3,
									Skill.SLAYER.getId());
							taskAmount--;
							return;
						}
					...
					case 7663:
						if (npc.inWild()) {
							...
							taskAmount--;
							player.getPA().addSkillXP(task.getExperience() * 3, Skill.SLAYER.getId());
							break;
						}
					}
						...
						for (int consecutiveInterval = 1; consecutiveInterval <= consecutiveTasks; consecutiveInterval++) {
							 ...
							 if (consecutiveTasks % 1000 == 0) {
              				...
              				return;
													...
													case 6797:
														player.getPA().addSkillXP(BOSS_TASK_EXPERIENCE, Skill.SLAYER.getId());
														player.sendMessage("You have completed a boss task and have gained an additional "
																+ Misc.insertCommas(Integer.toString(BOSS_TASK_EXPERIENCE)) + " experience."); 
														break;
													}
	...
	public void handleSuperiorExp(NPC npc) {
		task.ifPresent(task -> {
			...
			if (!name.equals(task.getPrimaryName())) {
				...
				if (isSuperiorNpc()) {
					player.getPA().addSkillXP(task.getExperience() * 10 * 3,
							Skill.SLAYER.getId());
	...
	public boolean onActionButton(int actionId) {
		...
		case 41012:
			...
			points -= 30;
			player.getPA().addSkillXP(amount, 18);
