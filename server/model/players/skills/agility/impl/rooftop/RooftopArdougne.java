...
public class RooftopArdougne {
	...
	public boolean execute(final Player c, final int objectId) {
			...
			CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
				...
				//	}
					...
					case 5:
						...
						c.getAgilityHandler().RoofAgilityProgress[8][0] = true;
						container.stop();
						break;
					}
				}
			...
			CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
				...
				//	}
					...
					case 7:
						...
						c.getAgilityHandler().RoofAgilityProgress[8][1] = true;
						container.stop();
						break;
					}
				}
		...
		case PLANK:
			...
			c.getAgilityHandler().RoofAgilityProgress[8][2] = true;
			return true;
			
		case JUMP_2ND_GAP:
			AgilityHandler.delayEmote(c, "JUMP", 2653, 3314, 3, 2);
			c.getAgilityHandler().RoofAgilityProgress[8][3] = true;
			return true;
			
		case JUMP_3RD_GAP:
			AgilityHandler.delayEmote(c, "JUMP_DOWN", 2651, 3309, 3, 2);
			c.getAgilityHandler().RoofAgilityProgress[8][4] = true;
			return true;
			
		case STEEP_ROOF:
			c.setForceMovement(2656, 3297, 0, 200, "SOUTH", 762);
			//c.getAgilityHandler().move(c, 3, -3, 762, -1);
			c.getAgilityHandler().RoofAgilityProgress[8][5] = true;
			return true;
			...
			CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					...
					if (c.getX() == 2658 && c.getY() == 3298) {
						...
						c.getAgilityHandler().roofTopFinished(c, 5, 950, 4000, 8);
