...
public class Herblore {
	...
	private void idHerb(int slot) {
		if (c.getItems().playerHasItem(information[slot][0])) {
			if (c.getSkills().getLevel(Skill.HERBLORE) >= information[slot][2]) {
				...
				c.getSkills().addExperience(information[slot][3] * Config.HERBLORE_EXPERIENCE, Skill.HERBLORE);
	...
	private void makePot(int herbId) {
		if (c.getItems().playerHasItem(227)
			...
			if (c.getSkills().getLevel(Skill.HERBLORE) >= information2[slot][3]) {
				...
				// }
				c.getSkills().addExperience(information2[slot][4] * Config.HERBLORE_EXPERIENCE, Skill.HERBLORE);
			} else {
				c.sendMessage("You need a herblore level of "
