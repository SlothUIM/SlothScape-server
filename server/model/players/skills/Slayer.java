...
public class Slayer {
	...
	public int[] lowTasks = { 1648, 1612, 117, 1265, 90, 103, 78, 119, 18, 101,
			1265, 181 };
		...
		for (int j = 0; j < lowTasks.length; j++) {
			if (lowTasks[j] == c.slayerTask) {
				c.sendMessage("You already have an easy task... to kill "
						+ c.taskAmount + " "
						+ World.getWorld().npcHandler.getNpcListName(c.slayerTask) + ".");
				return;
			}
		}
		giveTask(1);
	...
	public void giveTask(int taskLevel) {
		...
		c.sendMessage("You have been assigned to kill " + c.taskAmount + " "
				+ World.getWorld().npcHandler.getNpcListName(given)
				+ " as a slayer task.");
	}
