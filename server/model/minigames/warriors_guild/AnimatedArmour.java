package server.model.minigames.warriors_guild;
...
public class AnimatedArmour {
	...
	public static void itemOnAnimator(final Player player, int itemId) {
		...
		lastPlate = armour.getPlatebodyId();
		lastLegs = armour.getPlatelegsId();
		lastHelm = armour.getHelmId();
		player.getPA().walkTo(0, +3);
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {

			@Override
			public void execute(CycleEventContainer event) {
				//World.getWorld().getNpcHandler().spawnNpc(player, armour.getNpcId(), animator_west ? SPAWN_X : 2857, SPAWN_Y,
						//0, 1, armour.getHealth(), armour.getMax(), armour.getAttack(), armour.getDefence(), true, true);
	...
	public static int lastHelm, lastLegs, lastPlate;
	public static void dropTokens(Player player, int npcType, int x, int y) {
		...
		if (npc != null) {
			...
			//World.getWorld().getItemHandler().createGroundItem(player, lastHelm, x, y, player.getHeight(), 1, player.getIndex());
			//World.getWorld().getItemHandler().createGroundItem(player, lastLegs, x, y, player.getHeight(), 1, player.getIndex());
			//World.getWorld().getItemHandler().createGroundItem(player, lastPlate, x, y, player.getHeight(), 1, player.getIndex());
