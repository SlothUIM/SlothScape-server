	...
	public void cycle() {
		...
		CycleEventHandler.getSingleton().addEvent(this, new CycleEvent() {
			...
			public void execute(CycleEventContainer event) {
				...
				if (SkillcapePerks.ATTACK.isWearing(player) || SkillcapePerks.isWearingMaxCape(player)) {
					player.sendMessage("Your cape negates the need for warriors guild tokens.");
				} else {
					player.getItems().deleteItem2(8851, 10);
					player.sendMessage("You notice some of your warrior guild tokens disappear..");
				}
			}
	...
	public void dropDefender(int x, int y) {
		if (isActive() && Boundary.isIn(player, CYCLOPS_BOUNDARY) && !Boundary.isIn(player, WAITING_ROOM_BOUNDARY)) {
			...
			if (Misc.random(chance) == 0) {
				...
				player.sendMessage("@blu@The cyclops dropped a " + ItemAssistant.getItemName(item) + " on the ground.");
			}
		}
	}
