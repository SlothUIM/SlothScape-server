...
public class SkotizoLostItems extends ArrayList<Item> {
	...
	public void retain() {
		...
		if (!player.getItems().playerHasItem(995, price)) {
			...
			player.getDH().sendStatement("You need at least 500,000GP to claim your items.");
			return;
		}
		for (Item item : this) {
		...
		player.talkingNpc = 7283;
		//if (player.getMode().isUltimateIronman()) {
		//	player.getDH().sendStatement("You have retained all of your lost items for 500,000GP.", "Your items are in your inventory.",
		//			"@red@If there was not enough space, they were dropped.");
		//} else {
			player.getDH().sendStatement("You have retained all of your lost items for 500,000GP.", "Your items are in your bank.");
		//}
		player.nextChat = -1;
	}
