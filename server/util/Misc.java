...
//import server.content.gauntlet.RoomDirection;
...
public class Misc {
	...
	public static int textPack(String text, byte[] packed) {
	    if (text == null) {
	        packed[0] = 0;
	        return 1;
	    }
	    ...
	    for (int i = 0; i < text.length(); i++) {
	        int charCode = getCharCode(text.charAt(i));
	        if (charCode > 12) {
	            charCode += 195;
	        }
	        ...
	        if (carryOverNibble == -1) {
	            if (charCode < 13) {
	                carryOverNibble = charCode;
	            } else {
	                packed[pos++] = (byte) charCode;
	            }
	        } else if (charCode < 13) {
	            packed[pos++] = (byte) ((carryOverNibble << 4) + charCode);
	            carryOverNibble = -1;
	        } else {
	            packed[pos++] = (byte) ((carryOverNibble << 4) + (charCode >> 4));
	            carryOverNibble = charCode & 0xF;
	        }
	    }

	    if (carryOverNibble != -1) {
	        packed[pos++] = (byte) (carryOverNibble << 4);
	    }
	...
	private static int getCharCode(char c) {
	    // Mapping of valid chat characters to their RuneScape client code
	    final String validChars = " abcdefghijklmnopqrstuvwxyz0123456789.,!?/:;%&*()'-+=@#$^<>\"[]\\{}|~_";
	    int index = validChars.indexOf(c);
	    if (index == -1) {
	        return 0; // default to space if char not found
	    }
	    return index;
	}
	...
	public static String getWorldTime() {
	    // America/Chicago covers the Central US time zone automatically
	    return LocalTime.now(ZoneId.of("America/Chicago")).format(DateTimeFormatter.ofPattern("HH:mm")) + " CT";
	}
	...
	public static long playerNameToLong2(String s) {
	    long l = 0L;
	    int maxLength = Math.min(s.length(), 12);
	    s = s.toLowerCase();

	    for (int i = 0; i < maxLength; i++) {
	        char c = s.charAt(i);
	        int index = -1;
	        for (int j = 0; j < playerNameXlateTable.length; j++) {
	            if (playerNameXlateTable[j] == c) {
	                index = j;
	                break;
	            }
	        }
	...
	public static int getMoveDirection(int dx, int dy) {
	    if (dx < 0) {
	        if (dy < 0) return 0; // South-West
	        else if (dy > 0) return 5; // North-West
	        else return 3; // West
	    } else if (dx > 0) {
	        if (dy < 0) return 2; // South-East
	        else if (dy > 0) return 7; // North-East
	        else return 4; // East
	    } else {
	        if (dy < 0) return 1; // South
	        else if (dy > 0) return 6; // North
	        else return -1; // No movement
	    }
	}
