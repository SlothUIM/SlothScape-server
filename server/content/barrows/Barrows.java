...
//import server.content.achievement_diary.morytania.MorytaniaDiaryEntry;
import server.content.barrows.brothers.Ahrim;
import server.content.barrows.brothers.Brother;
import server.content.barrows.brothers.Dharok;
import server.content.barrows.brothers.Guthan;
import server.content.barrows.brothers.Karil;
import server.content.barrows.brothers.Torag;
import server.content.barrows.brothers.Verac;
import server.world.Boundary;
//import server.model.player.GlobalMessages;
...
public class Barrows {
	...
	public void test() {
		...
		for (int i = 0; i < 10; i++) {
			player.getPA().itemOnInterface(player, 36754, i, -1, -1);
		}
		...
		getLastBrother().ifPresent(brother -> {
			if (!completed) {
				if (!brother.isDefeated()) {
					...
					for (int i = 0; i < 10; i++) {
						player.getPA().itemOnInterface(player, 36754, i, -1, -1);
					}//itemOnInterface(Player player, int frame, int slot, int id, int amount)
					giveRewards(rewardList);
	...
	private void giveRewards(RewardList rewardList) {
		...
		int gotBarrows = 0;
		int spot = 0;
		player.getPA().showInterface(36750);
		for (RewardItem item : randomRewards) {
			...
			//player.getPA().sendFrame34(itemId, amount, 36754, spot);
			player.getPA().itemOnInterface(player, 36754, spot, itemId, amount);
			spot++;
			if (item.getRarityLevel() == RewardLevel.RARE) {
				...
				gotBarrows = 1;
				CollectionLogData barrowsLog = player.getCollectionLog().registry.getLog("Barrows");
			    int item2 = itemId;
			    player.getPA().CollLogPopUp(barrowsLog.getEntries(), item2, 1);  // Or better, add a method in BarrowsLog to handle this
			    barrowsLog.addItem(item2, 1);  // if you add this method to your log classes
			}
